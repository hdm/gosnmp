...
func withUnconnectedSocket(t *testing.T, enable bool) {
	...
	addr := srvr.LocalAddr().(*net.UDPAddr).IP
...
func pcapgz2bytes(filename string) ([][]byte, error) {
	res := [][]byte{}
	...
	zrw, err := gzip.NewReader(inp)
	if err != nil {
		return nil, err
	}
	defer zrw.Close()

	r, err := pcapgo.NewReader(zrw)
	if err != nil {
		return nil, err
	}
	for {
		raw, ci, err := r.ZeroCopyReadPacketData()
		if err != nil {
			break
		}
		...
		packet := gopacket.NewPacket(raw, r.LinkType(), gopacket.DecodeOptions{Lazy: true, NoCopy: true})
